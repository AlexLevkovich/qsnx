...
bool KPtyDevicePrivate::_k_canRead()
{
    ...
    if (!::ioctl(q->masterFd(), PTY_BYTES_AVAILABLE, (char *)&available)) {
        ...
        if (readBytes < 0) {
            ...
            q->setErrorString(QObject::tr("Error reading from PTY"));
            return false;
        }
...
bool KPtyDevicePrivate::_k_canWrite()
{
    ...
    if (wroteBytes < 0) {
        q->setErrorString(QObject::tr("Error writing to PTY"));
        return false;
    }
...
bool KPtyDevicePrivate::doWait(int msecs, bool reading)
{
    ...
    while (reading ? readNotifier->isEnabled() : !writeBuffer.isEmpty()) {
        ...
        case -1:
            ...
            return false;
        case 0:
            q->setErrorString(QObject::tr("PTY operation timed out"));
            return false;
        default:
...
bool KPtyDevice::open(OpenMode mode)
{
    ...
    if (!KPty::open()) {
        setErrorString(tr("Error opening PTY"));
        return false;
    }
